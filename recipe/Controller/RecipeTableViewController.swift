    ...
    func searchForRecipesByIngredients() {
        ...
        provider.request(.findRecipesByIngredients(ingredients: ingredients)) {
            ...
            case .failure(let error):
                let alert = UIAlertController(title: "Failure", message: "Please check your internet connection.", preferredStyle: UIAlertController.Style.alert)
        ...
        provider.request(.getRecipesByName(dishName: dishName)) {
            ...
            case .success(let response):
                do {
                    ...
                    let alert = UIAlertController(title: "Oops! Try again.", message: "There seems be an error with the server. Please try again later.", preferredStyle: UIAlertController.Style.alert)
                    alert.addAction(UIAlertAction(title: "Cancel", style: UIAlertAction.Style.cancel, handler: nil))
                    self.present(alert, animated: true, completion: nil)
                    print(error.localizedDescription)
                    print(error.localizedDescription)
